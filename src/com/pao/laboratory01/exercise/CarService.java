...
//    public static void afisare_masini(){
//        if car
//    }
//
    ...
    public void addReview(String carName, String review) {
        for(var x : cars){
            if(x.getName().equals(carName)){
//                System.out.println("Executat");
                Car a=x;
                String [] rev = new String[x.getReviews().length+1];
                System.arraycopy(x.getReviews(), 0, rev, 0, x.getReviews().length);
                x.setReviews(rev);
                x.getReviews()[x.getReviews().length-1]=review;
                System.out.println("Totul e ok");
                return;
            }
        }
        System.out.println("Totul nu e ok");
